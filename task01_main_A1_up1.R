...
X <- as.data.frame(XA1) %>% as.matrix %>% scale
y <- as.data.frame(yA1) %>% as.matrix %>% as.numeric()
colnames(y) <-  "y"
...
model <- randomForest(X, y, ntree=500, importance=T, 
                      keep.forest=T, replace = F, 
                      sampsize = 0.5*nrow(X))
plot(model)
varImpPlot(model, type=1, main = "% of increasing in MSE, Dataset A1")
